...
-- PostgreSQL does not support CREATE POLICY IF NOT EXISTS; use a DO block.
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'daily_digest_log'
    AND policyname = 'users can read own digest log'
  ) THEN
